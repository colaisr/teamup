...
- **Attention surface:** **`/attention`** now mirrors the manager workflow pattern: auto-load from active workspace, KPI summary strip, search/filter/sort controls, semantic risk badges, explicit reasons/suggested-action blocks, and per-task AI explain output inline. It now also renders **severity**, **loop count**, and structured **signal-level diagnostics** (threshold/observed context) from the upgraded Phase 7 payload.
...
- **Dashboard surface:** **`/dashboard`** now auto-loads active-workspace metrics with KPI cards, risk-map interpretation blocks, manager-oriented “next actions”, and expanded storytelling strips: **status bottlenecks** (time-in-status), **task-type breakdown**, and **weekly trend strip** sourced from Phase 6 aggregation payloads.
- **Task-type data hygiene:** analytics now normalizes legacy/noisy `task_type` values (including raw custom-field JSON artifacts from older imports) into stable buckets, so dashboard grouping remains readable. Non-computable cycle/flow slices show explicit “no data” instead of misleading zeroes.
- **App surfaces:** core manager pages (**`/dashboard`**, **`/tasks`**, **`/attention`**, **`/impact`**, **`/settings/integrations`**) now follow the shared Product -> UX -> UI standard (content-first, responsive, drilldown-oriented). Remaining gaps are mostly release hygiene/pilot execution and optional MVP+ capabilities, not baseline page usability.
