...
- **Dashboard** depth (bottleneck cards, filters, richer trends), optional **Impact** extras (export/share, MVP+ **AI-assisted** impact narrative), PostgreSQL-heavy CI parity, and broader MVP+ AI capabilities (task-quality scoring, workspace narratives, full contextual chat sessions) — see [`MVP_IMPLEMENTATION_PLAN.md`](MVP_IMPLEMENTATION_PLAN.md) cross-cutting notes. **Heavyweight background job platforms and deep automated retry ladders are deferred past MVP closure** (same plan §3).
