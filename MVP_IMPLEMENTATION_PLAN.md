...
| **8 — Dashboard UI** | **Partial** | **`/settings/integrations`**: connection **list**; **add / edit / sync / full sync / disconnect**; full **wizard** (credentials with verify + loading states, team/Space refresh, mapping, done); **sync_mode** controls (incremental/full) + sync error visibility; **UTC timestamps shown in browser local time**. Routes **`/dashboard`**, **`/attention`**, **`/impact`**, onboarding and **`/tasks`**. `/tasks` is now a manager-grade operations surface (auto-load from active workspace, summary strip, collapsible advanced filters, responsive hierarchy cards, chevron tree expand, richer details drawer with deterministic insight + AI action), replacing the previous debug-heavy wide table approach. Shell/sidebar UX also improved with viewport-safe sticky behavior and collapse/expand rail mode so footer controls stay reachable. If ClickUp Time in Status is unavailable on the current plan, the details panel shows a localized provider-plan limitation message. Core `/tasks` + AI assistant chrome use Russian-first `t()` keys. Workspace/members stay under **user settings** (Phase 1). **Still open:** richer bottleneck cards / trend parity on dashboard/attention. |
| **9 — Value / Impact** | **Partial** | Live vs latest **baseline**; first baseline after import when mappings exist; manual baseline/current; **history API** + raw table; **SVG trends**; optional **`weekly`** snapshots (`IMPACT_WEEKLY_SNAPSHOT_*`). **UI:** pilot-oriented block (**`impact.pilot*`** / **`impact.narrative*`** copy), improved/worsened **summary strips**, **single comparison table** (metric, trend, baseline, current, Δ, %) with row tinting—replacing stacked per-metric cards. **Still polishable:** export/share, denser “executive” summary card, AI-assisted impact narrative (MVP+). |
...
Cross-cutting gaps vs DoD MVP: stricter §3.1 i18n quality automation (missing-key checker + remaining non-core literal cleanup); **dashboard/attention** depth (bottleneck cards, deeper trend strips) and any extra **Impact** presentation (export, MVP+ AI narrative); Postgres-backed parity tests where needed. **UI consistency standard established from `/tasks` rework:** Product intent -> UX structure -> UI fit/comfort (content-first, collapsible advanced controls, responsive non-overflow lists, explicit drilldown). Apply this standard across remaining pages during polish. **Post-MVP (when scaling traffic):** replace in-process scheduler threads with queue/worker deployments and deepen retry/backoff—not listed as MVP exit blockers here. For **MVP+ AI**: task-quality scoring, workspace/impact narratives, full chat session lifecycle.
...
5. **Consistent page rework pass (Product -> UX -> UI)** — Reapply the `/tasks` standard across **`/dashboard`**, **`/attention`**, and next polish passes: define manager decision intent first, auto-load from active workspace, keep advanced controls collapsible, avoid horizontal overflow on laptop widths, prioritize clear drilldowns over dense debug surfaces.
6. **§3.1 i18n quality gate** — Script or CI check for missing `ru`/`en` keys; purge remaining stray literals outside localized surfaces.
7. **Pilot readiness (Phase 10)** — When **dashboard / attention depth (3–5)** matches the story you want for pilot, execute `docs/PILOT_RUNBOOK.md`, intervention logging, exit review.
8. **AI capability rollout on new foundation (Phase 8.5)** — Keep deterministic context + evidence refs mandatory; add **workspace takeaways**, **impact narrative**, **task description quality** atop `ai_runs`/`ai_outputs` + `/api/ai`.
...
If you must pick **one** next milestone: **dashboard depth / metrics & attention (3–4)** to support the pilot story now that **`/impact`** has a pilot narrative shell and comparison table; or start **§3.1 i18n gate (5)** if release hygiene is the priority.
