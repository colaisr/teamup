...
4. **Analytics storytelling depth on reworked pages (Phase 8)** — Keep the new manager-first UI shells, then add deeper bottleneck/time-in-status slices and trend narratives where pilot decisions need them.
...
6. **Pilot readiness (Phase 10)** — With core pages now reworked, execute `docs/PILOT_RUNBOOK.md` against real interventions, validate KPI movement cadence, and tune thresholds/copy from pilot feedback.
...
If you must pick **one** next milestone: **analytics storytelling depth + metrics/attention completeness (3–4)** on top of the new UI baseline; or start **§3.1 i18n gate (5)** if release hygiene is the priority.
