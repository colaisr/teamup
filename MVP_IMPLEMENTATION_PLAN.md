...
Cross-cutting gaps vs DoD MVP: stricter §3.1 i18n quality automation (missing-key checker + remaining non-core literal cleanup); **dashboard** depth (bottleneck cards, time-in-status strips) and any extra **Impact** presentation (export, MVP+ AI narrative); Postgres-backed parity tests where needed. **Post-MVP (when scaling traffic):** replace in-process scheduler threads with queue/worker deployments and deepen retry/backoff—not listed as MVP exit blockers here. For **MVP+ AI**: task-quality scoring, workspace/impact narratives, full chat session lifecycle.
...
6. **Pilot readiness (Phase 10)** — When **dashboard / attention depth (3–4)** matches the story you want for pilot, execute `docs/PILOT_RUNBOOK.md`, intervention logging, exit review.
