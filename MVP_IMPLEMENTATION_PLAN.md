...
| **8 — Dashboard UI** | **Partial** | Manager-grade UI pass is now applied across **`/tasks`**, **`/settings/integrations`**, **`/attention`**, and **`/dashboard`**: auto-load from active workspace, summary KPI strips, responsive non-overflow card/list layouts, clear risk/status semantics, and progressive disclosure (filters/details/history). `/tasks` moved from debug table to hierarchy-aware operations workspace (chevron tree expand, richer details drawer, deterministic insight + AI action). `/settings/integrations` now has clearer readiness/sync/error scanning + quick filters and improved action hierarchy. `/attention` and `/dashboard` now provide manager-first scanability (risk buckets, actionable next steps) instead of raw input-first debug forms. Shell/sidebar remains viewport-safe with collapse/expand rail mode; global dark/light theme toggle is available in user settings and applied app-wide. If ClickUp Time in Status is unavailable on the current plan, task details show localized provider-plan limitation messaging. **Still open:** deeper analytics content parity (bottleneck/time-in-status strips, richer trends tied to expanded metrics API breadth). |
...
3. **Analytics storytelling depth on reworked pages (Phase 8)** — Consume the expanded Phase 6/7 backend payloads in UI: bottleneck/time-in-status drilldowns, task-type and weekly trend strips, and clearer manager narratives where pilot decisions need them.
4. **§3.1 i18n quality gate** — Script or CI check for missing `ru`/`en` keys; purge remaining stray literals outside localized surfaces.
5. **Pilot readiness (Phase 10)** — With core pages now reworked and Phase 6/7 engines upgraded, execute `docs/PILOT_RUNBOOK.md` against real interventions, validate KPI movement cadence, and tune thresholds/copy from pilot feedback.
6. **AI capability rollout on new foundation (Phase 8.5)** — Keep deterministic context + evidence refs mandatory; add **workspace takeaways**, **impact narrative**, **task description quality** atop `ai_runs`/`ai_outputs` + `/api/ai`.
...
If you must pick **one** next milestone: **analytics storytelling depth on top of the completed Phase 6/7 engines (3)**; or start **§3.1 i18n gate (4)** if release hygiene is the priority.
