...
2. **Scheduled sync + retries (Phase 3)** — Current manual `sync_mode` import + observability is in place; next add worker orchestration (Cron/single worker before full Celery), retry policy/backoff, and stale-sync alerts.
