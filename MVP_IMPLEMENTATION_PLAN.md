...
| **3 — Ingestion** | **Partial** | **On-demand import** supports modes: **`auto`** (incremental after `last_synced_at` with overlap), **`incremental`**, **`full`** (~90 days by `date_created_gt`) for list/space sources. Import writes `Task`, `TaskTransition`, `ClickUpRawEvent`; hardens ClickUp status parsing (string/object) and stores custom field snapshot in `tasks.task_type` (**TEXT** on PostgreSQL). **Sync observability shipped** on connection (`last_sync_attempt_at`, `last_sync_error`, `last_synced_at`) and surfaced in integrations UI with incremental/full controls. **Still open:** scheduled worker/Celery and deeper retry/QC automation. |
...
| **8 — Dashboard UI** | **Partial** | **`/settings/integrations`**: connection **list**; **add / edit / sync / disconnect**; full **wizard** (credentials with verify + loading states, team/Space refresh, mapping, done); **sync_mode** controls (incremental/full) + sync error visibility; **UTC timestamps shown in browser local time**. Routes **`/dashboard`**, **`/attention`**, **`/impact`**, onboarding and new **`/tasks`** page (debug/ops list with hierarchy, subtree attention, details slide-over + transition timeline) — workspace/members under **user settings** (Phase 1). **No** full bottleneck cards / filters / weekly trend parity yet. |
